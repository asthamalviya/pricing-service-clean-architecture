...
**Date:** August 2026  
**Word Count:** 7,437 words (Main Report including Sections 1–5 and Appendices A–D; excludes references per university guidelines)
...
The first quality gap is therefore **high coupling and lack of separation of concerns**. Because pricing and infrastructure are contained in the same handler, a change to pricing rules also changes a component responsible for HTTP and persistence. This increases the regression surface and means the business calculation cannot be reused independently of the API. From a business perspective, this increases the cost and risk of introducing new pricing functionality. Within the software development lifecycle, the v0 design forces the natural testing boundary to be the HTTP API endpoint, which means defects in pricing logic can only be detected through integration-level testing that requires framework setup and fixtures. This delays feedback and increases the cost of debugging compared to unit-testable business logic.
...
This is not merely a stylistic choice. The absent patterns create a structural quality problem where business logic cannot be exercised, tested or evolved independently of infrastructure. As shown in **Figure 2** (Appendix D), v1 restructures these concerns into focused components. The diagnosis from Section 1 therefore points toward explicit architectural decisions that establish these boundaries.
...
**Pattern Rejection Decision Matrix**

| Pattern | Problem It Claims to Solve | Why It Was Rejected | What Was Used Instead |
|---------|---------------------------|-------------------|----------------------|
| **Abstract Factory** | Encapsulate plan object creation with polymorphism | Plans differ only in configuration (unit price), not behaviour. Creating separate classes and a factory would add indirection without removing meaningful variation. | `PlanPricer` with configuration lookup—simpler and reflects actual variation (data, not behaviour) |
| **Observer** | Decouple invoice persistence from pricing calculation through event publishing | Only one synchronous consumer exists (invoice persistence). Introducing event infrastructure would add complexity without solving a demonstrated business requirement. | Repository Pattern—provides the same decoupling with less overhead, and makes the single flow explicit. |
| **Builder** | Provide controlled construction and validation of `QuoteRequest` objects | Pydantic framework already provides typed model construction, field validation and error reporting at the API boundary. Introducing a separate Builder would duplicate framework functionality. | Pydantic `QuoteRequest` model with FastAPI validation—framework handles construction and validation directly. |
...
The 24/25 boundary also demonstrates an important distinction between **testing correctness and determining business correctness**. As shown in **Figure 4** (Appendix D), the test correctly detects that 25 seats cost less than 24 (£675 vs £684). The test itself cannot decide whether that behaviour is a defect. It provides evidence for the business to evaluate.
...
This design approach—testing at the precise points where logic changes—is consistent with proactively designing tests that would detect mutations most likely to occur in the implementation (Jia & Harman, 2011). The test suite therefore provides evidence of adequacy beyond simple coverage percentage, since the tests target specific logical decision points where defects are most likely to be introduced.
...
### 4.4 Collaboration and Peer Review: Application to a Team Context

This project was completed as an individual assignment. No formal peer review, team meetings or collaborative decision-making occurred during development. However, the architectural approach is deliberately structured to support peer review and collaborative decision-making in a professional team setting.

**How ADRs Support Team Collaboration (in a real team context):**
